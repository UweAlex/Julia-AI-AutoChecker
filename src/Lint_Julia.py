...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        print("Usage: python lint_julia_v6.py [file.jl] or | python lint_julia_v6.py -")import re
from collections import Counter
...
def lint_julia(code):
    ...
    # NEW 10. Detect invalid @test keywords (e.g., label= or description=)
    invalid_test_keywords = re.findall(r'@test\s+.*?\s+(label|description)\s*=\s*', code)
    if invalid_test_keywords:
        issues.append(f"Invalid @test keywords: {set(invalid_test_keywords)} – @test does not support label= or description=; use string suffix after test expression")

    # NEW 11. Unbalanced parentheses
    left_paren = code.count('(')
    right_paren = code.count(')')
    if left_paren != right_paren:
        issues.append(f"Unbalanced parentheses: {left_paren} '(' vs {right_paren} ')'")
    
    # NEW 12. Unnecessary $(var) in strings (suggest $var)
    unnecessary_interp = re.findall(r'\$\(\w+\)', code)
    if unnecessary_interp:
        issues.append(f"Unnecessary $(var) in strings: {unnecessary_interp[:3]}... – use $var in normal strings")
